...
export class Entrypoint {
  ...
  public handle() {
    ...
    console.info(
      `Parsed argv with interval as ${chronoTypeInput.interval} and ordinal as ${chronoTypeInput.ordinal}`,
    );
    ...
    // 4. Retrieve required ChronoNote config and check if valid
    const intervalVars = this.configProvider.getIntervalConfig(
      chronoNote.getInterval(),
    );
...
export function createEntrypoint(
  ...
  return new Entrypoint(configProvider, fileProvider, customDateTime);
}
